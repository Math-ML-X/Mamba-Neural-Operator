...
from .attention_module_st import CrossAttention
...
class CrossFormer(nn.Module):
    def __init__(self,
        ...
        #                                           )
        self.cross_attn_module = CrossAttention(dim, attn_type,
                                                       heads=heads, dim_head=dim_head, dropout=dropout,
                                                       relative_emb=relative_emb,
                                                       scale=scale,
                                                       relative_emb_dim=relative_emb_dim,
...
class PointWiseDecoder2D(nn.Module):
    def __init__(self,
        ...
        self.decoding_transformer = CrossFormer(self.latent_channels//2, 'standard', 4,
...
class PointWiseDecoder1D(nn.Module):
    ...
    def rollout(self,
        ...
        # forward the dynamics in the latent space
        for step in range(forward_steps):
            u = self.forward(z, propagate_pos, input_pos)
...
class PointWiseDecoder2DSimple(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoding_transformer = CrossFormer(self.latent_channels, 'standard', 4,
                                                self.latent_channels, self.latent_channels,
                                                use_ln=False,
                                                residual=True,
                                                relative_emb=True,
