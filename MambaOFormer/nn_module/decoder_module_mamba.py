...
from .mamba_ops import SS2D, CrossSS2D
...
class CrossFormer(nn.Module):
    def __init__(self,
        ...
        config_mamba = {}
        config_mamba["ssm_d_model"] = dim
        config_mamba["ssm_d_state"] = 16
        config_mamba["ssm_ratio"] = 2.0
        config_mamba["ssm_rank_ratio"] = 2.0
        config_mamba["ssm_dt_rank"] = "auto"
        config_mamba["ssm_act_layer"] = "silu"
        config_mamba["ssm_conv"] = 0
        config_mamba["ssm_conv_bias"] = False
        config_mamba["ssm_drop_rate"] = 0.0
        config_mamba["ssm_simple_init"] = False
        config_mamba["softmax_version"] = False
        config_mamba["forward_type"] = "v2"
        config_mamba["share_proj"] = False

        config_mamba = DictAsObj(config_mamba)

        self.cross_attn_module = CrossSS2D(config_mamba)
    ...
    def forward(self, x, z, x_pos=None, z_pos=None):
        ...
        if self.use_ln:
            z = self.ln1(z)
            if self.residual:
                x = self.ln2(self.cross_attn_module(x, z)) + x
            else:
                x = self.ln2(self.cross_attn_module(x, z))
        else:
            if self.residual:
                x = self.cross_attn_module(x, z) + x
            else:
                x = self.cross_attn_module(x, z)
