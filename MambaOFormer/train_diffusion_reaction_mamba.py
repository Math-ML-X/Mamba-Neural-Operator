        ...
        while True:
            ...
            if (opt.eval_mode or ((n_iter - 1) % opt.ckpt_every == 0 or n_iter >= opt.iters)) and (local_rank == 0):
                ...
                with torch.no_grad():
                    for j, data in enumerate(tqdm(test_dataloader)):
                        ...
                        if j == 0 and opt.is_vis:
                            x_out_all_array = x_out_all.cpu().numpy().reshape(-1, 128, 128, 101, 2)[0]
                            gt_full_array = gt_full.cpu().numpy().reshape(-1, 128, 128, 101, 2)[0]
                ...
                if not opt.eval_mode and local_rank == 0:
                    ...
                    save_checkpoint(ckpt, os.path.join(checkpoint_dir, f'dr2d_latest_model.ckpt'))
                    save_checkpoint(ckpt, os.path.join(checkpoint_dir, f'dr2d_{n_iter}_model.ckpt'))
