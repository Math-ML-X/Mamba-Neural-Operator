...
class FullModel(nn.Module):
    ...
    def forward(self, x, input_pos, prop_pos):
        x = torch.cat((x, input_pos), dim=-1)
        z = self.encoder(x, input_pos)
        out = self.decoder(z, prop_pos, input_pos)
        return out
        ...
        if opt.eval_mode:
            ...
            with torch.no_grad():
                for j, data in enumerate(tqdm(test_dataloader)):
                    ...
                    print("Evaluating model complexity on first real batch...")


                    full_model = FullModel(encoder, decoder).cuda().eval()

                    x_real = (x - x_mean) / x_std
                    x_real = rearrange(x_real, 'b h w c -> b (h w) c')
                    y_real = rearrange(y, 'b h w c -> b (h w) c')
                    input_pos = grid.repeat([x.shape[0], 1, 1])
                    prop_pos = grid_query.repeat([x.shape[0], 1, 1])

                    with torch.no_grad():
                        inputs = (x_real, input_pos, prop_pos)

                        # FLOPs
                        flops = FlopCountAnalysis(full_model, inputs)
                        print(flops.by_module())
                        print(f"[Total FLOPs] {flops.total()/1e9:.3f} GFLOPs")

                        # Params
                        param_info = parameter_count(full_model)
                        total_params = sum(param_info.values())
                        ...
                        avg_time = (end_time_eval - start_time_eval) * 1000 / 100
                        max_mem = torch.cuda.max_memory_allocated() / (1024 ** 3)
                        print(f"[Avg Inference Time / 100 rounds] {avg_time:.2f} ms")
                        print(f"[Max GPU Memory Usage (1st run)] {max_mem:.2f} GiB")
