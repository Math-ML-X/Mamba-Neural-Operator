...
from util_metrics import eval_2d, eval_2d_time
...
def get_arguments(parser):
    ...
    parser.add_argument(
        '--in_channels', type=int, default=22, help='Channel of input feature. (default: 3)'
    )
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    sample_dir = os.path.join(opt.log_dir, 'samples_dr2d', task_name)
    if local_rank == 0:
    ...
    t, a = x_train.shape[1], x_train.shape[2]  # t=101, a=2
    ...
    x_mean = torch.mean(x_train).unsqueeze(0)  # [2, t_in, hw]
    x_std = torch.std(x_train).unsqueeze(0)  # [2, t_in, hw]

    y_mean = torch.mean(y_train).unsqueeze(0)  # [2, t_out, hw]
    y_std = torch.std(y_train).unsqueeze(0)  # [2, t_out, hw]
    ...
    with tqdm(total=opt.iters) as pbar:
        ...
        if opt.eval_mode:
            ...
            for metric_name in metric_list:
                ...
                for i in range(opt.out_seq_len + opt.in_seq_len):
            ...
            with torch.no_grad():
                for j, data in enumerate(tqdm(test_dataloader)):
                    ...
                    metrics_time = eval_2d_time(x_out_all, gt_full, metric_list=metric_list, length=opt.out_seq_len + opt.in_seq_len)
            ...
            for i in range(opt.out_seq_len + opt.in_seq_len):
                for metric_name in metric_list:
                    metric_time_ave_dict[metric_name][f't_{i}'] = np.mean(metric_time_dict[metric_name][f't_{i}'])
                    metric_time_std_dict[metric_name][f't_{i}'] = np.std(metric_time_dict[metric_name][f't_{i}'])
