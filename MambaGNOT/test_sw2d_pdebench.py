...
def validate_epoch(model,
    ...
    for _, data in enumerate(valid_loader):
        with torch.no_grad():
            ...
            gt = rearrange(g.ndata['y'], 'bn (t c) 1 -> bn t c', t=in_seq_len + out_seq_len, c=1)
            out = rearrange(out, 'bn (t c) -> bn t c', t=out_seq_len, c=1)
            ...
            if _ > 0:
                break
            if is_vis:
                y_pred_array = y_pred.cpu().numpy().reshape(-1, 128, 128, 101, 1)[0]
                y_array = y.cpu().numpy().reshape(-1, 128, 128, 101, 1)[0]
