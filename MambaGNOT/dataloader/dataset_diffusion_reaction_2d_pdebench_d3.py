...
class DiffusionReaction2DDataset(Dataset):
    ...
    def _prepare(self):
        ...
        v_sampled_inseq = rearrange(v_sampled[:, :self.in_seq, :], 'n t a -> n (t a)')
        feat = torch.cat((grid,
                          u_sampled_inseq,
                          v_sampled_inseq,
                          ), dim=1)
    ...
    def _update_dataset_config(self):
        self.config = {
            ...
            'theta_dim': 0,
            'output_dim': 182,  # output channel along time
            'output_data_chnl': 2,  # output channel
            'branch_sizes': [22],
            'branch_data_chnl': 2,  # input data channel
            'in_seq': self.in_seq,
