...
class DarcyFlowDataset(Dataset):
    ...
    def __init__(self, dataset_params, split):
        ...
        self.reduced_resolution_query = dataset_params['reduced_resolution_query'] if 'reduced_resolution_query' in dataset_params else dataset_params['reduced_resolution']
    ...
    def _prepare(self):
        ...
        # undersample on time and space
        a_query_sampled = a_query_sampled[::self.reduced_resolution_query, ::self.reduced_resolution_query]
        u_query_sampled = u_query_sampled[::self.reduced_resolution_query, ::self.reduced_resolution_query]
        ...
        feat_query = torch.cat((u_query_sampled,
                                ), dim=1)

        # add to the graph
        g = dgl.DGLGraph()
        g.add_nodes(feat.shape[0])
        g.ndata['x'] = grid_query
        g.ndata['y'] = feat_query
        self.graph = g

        input_f = MultipleTensors([feat])
        # input_f = feat
        self.input_f = input_f

        self.u_p = torch.tensor([0])
    ...
    def _load_sample(self, data_path):
        ...
        self.meshgenerator_query = SquareMeshGenerator(real_space=self.real_space,
                                                       mesh_size=self.mesh_size,
                                                       downsample_rate=self.reduced_resolution_query,
                                                       is_diag=False)
    ...
    def __getitem__(self, idx):
        ...
        return self.graph, self.u_p, self.input_f
    ...
    def _update_dataset_config(self):
        self.config = {
            'input_dim': 3,
            'theta_dim': 0,
            'output_dim': 1,  # output channel along time
            'output_data_chnl': 1,  # output channel
            'branch_sizes': [3],
            'branch_data_chnl': 1,  # input data channel
        }
