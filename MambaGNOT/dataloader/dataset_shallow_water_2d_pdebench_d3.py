...
from utils import UnitTransformerWithMeanStd, MultipleTensors
...
class ShallowWater2DDataset(Dataset):
    ...
    def __init__(self, dataset_params, split):
        ...
        self.normalize_x = dataset_params['normalize_x']
    ...
    def _update_dataset_config(self):
        self.config = {
            'input_dim': 3,  # position and u_p
            'theta_dim': 0,
            'output_dim': 91,  # output channel along time
    ...
    def __normalize_y(self):
        if self.y_normalizer is None:
            if self.normalize_y == 'unit':
                self.y_normalizer = UnitTransformerWithMeanStd(mean=self.u_mean, std=self.u_std)
                print('Target features are normalized using unit transformer')
                print(self.y_normalizer.mean, self.y_normalizer.std)

        self.graph.ndata['y'] = self.y_normalizer.transform(self.graph.ndata["y"], inverse=False)  # a torch quantile transformer
