...
class MambaMoEGPTConfig():
    ...
    def __init__(self,attn_type='linear', embd_pdrop=0.0, resid_pdrop=0.0,attn_pdrop=0.0, n_embd=128, n_head=1, n_layer=3, block_size=128, n_inner=4,act='gelu',n_experts=2,space_dim=1,branch_sizes=None,n_inputs=1):
        ...
        self.ssm_d_model = n_embd
        self.ssm_d_state = 16
        self.ssm_ratio = 2.0
        self.ssm_rank_ratio = 2.0
        self.ssm_dt_rank = "auto"
        self.ssm_act_layer = "silu"
        self.ssm_conv = 0
        self.ssm_conv_bias = False
        self.ssm_drop_rate = 0.0
        self.drop_path_rate = 0.0
        self.forward_type = "v2"
        self.downsample_version = "none"
        self.ssm_simple_init = False
        self.softmax_version = False
        self.share_proj = False


class MambaMoEGPTConfigABL():
    """ base GPT config, params common to all GPT versions """
    def __init__(self,attn_type='linear', embd_pdrop=0.0, resid_pdrop=0.0,attn_pdrop=0.0, n_embd=128, n_head=1, n_layer=3, block_size=128, n_inner=4,act='gelu',n_experts=2,space_dim=1,branch_sizes=None,n_inputs=1,
...
'''
class MIOEMambaBlock(nn.Module):
    def __init__(self, config):
        super(MIOEMambaBlock, self).__init__()
        ...
        print('Using Mamba')
        from models.mamba_ops import SS2D, CrossSS2D
        self.selfattn = SS2D(config)
        self.crossattn = CrossSS2D(config)
...
'''
    ...
    Trunck Net: geom
'''
class MambaGNOT_ANALYSIS(nn.Module):
    def __init__(self,
                 trunk_size=2,
                 branch_sizes=None,
                 space_dim=2,
        ...
        super(MambaGNOT_ANALYSIS, self).__init__()
        ...
        self.space_dim = space_dim
        self.gpt_config = MambaMoEGPTConfig(attn_type=attn_type,
        ...
        self.blocks = nn.Sequential(*[MIOEMambaBlock(self.gpt_config) for _ in range(self.gpt_config.n_layer)])

        self.out_mlp = MLP(n_hidden, n_hidden, output_size, n_layers=mlp_layers)

        # self.apply(self._init_weights)

        self.__name__ = 'MambaMIOEGPT'
...
class MambaGNOT_ANALYSIS_ABL(nn.Module):
    def __init__(self,
                 trunk_size=2,
                 branch_sizes=None,
                 space_dim=2,
        ...
        super(MambaGNOT_ANALYSIS_ABL, self).__init__()
        ...
        self.output_size = output_size
        self.space_dim = space_dim
        self.gpt_config = MambaMoEGPTConfigABL(attn_type=attn_type,
                                            embd_pdrop=ffn_dropout,
                                            resid_pdrop=ffn_dropout,
                                            attn_pdrop=attn_dropout,
                                            n_embd=n_hidden,
