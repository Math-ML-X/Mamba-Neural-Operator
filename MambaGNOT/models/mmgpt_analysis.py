...
class MIOECrossAttentionBlock(nn.Module):
    def __init__(self, config):
        ...
        act_map = {'gelu': GELU, 'tanh': Tanh, 'relu': ReLU, 'sigmoid': Sigmoid}
        self.act = act_map[config.act]
        ...
        self.moe_mlp1 = nn.ModuleList([nn.Sequential(
            nn.Linear(config.n_embd, config.n_inner), self.act(),
            nn.Linear(config.n_inner, config.n_embd)) for _ in range(self.n_experts)])
...
class GNOT_ANALYSIS(nn.Module):
    def __init__(self,
        ...
        super(GNOT_ANALYSIS, self).__init__()
        ...
        self.space_dim = space_dim
        self.gpt_config = MoEGPTConfig(attn_type=attn_type,
